...
import { OPENING_LEVELS } from './opening-levels.mjs?v=20260916a';
...
// erratic. Deeper mines introduce more hunters and quicker spiders.
export const ENEMY = {
...
function makePlayer(id, corner, opts = {}) {
  return {
    ...
    cpu: !!opts.cpu,
    name: opts.name || 'Miner',
    color: opts.color || 'blue',
    wins: 0,
    ai: null,
    bombCooldown: 0,
    walkPhase: 0,
    decideAtCentre: !!opts.cpu,
    atNode: false,
  };
}
...
function makeEnemy(type, x, y, rng) {
  return {
    ...
    x: x + 0.5,
    y: y + 0.5,
    dir: DIR_NAMES[Math.floor(rng() * 4)],
    alive: true,
    deadAt: -1,
    walkPhase: 0,
    moving: true,
...
export function createGame(opts = {}) {
  ...
  if (mode === 'adventure') {
    ...
    state.players = [
      ...
      makePlayer(1, CORNERS[1], {
        name: opts.p2Name || (opts.cpu ? 'The computer' : 'Red'),
        color: 'red',
        cpu: !!opts.cpu,
      }),
    ];
...
export function startLevel(state, level) {
  ...
  state.level = level;
  const { grid, bricks } = lesson
    ? { grid: new Uint8Array(W * H), bricks: [] }
    : buildGrid(state.rng, spec.density, [CORNERS[0]]);
  state.grid = grid;
  state.projectiles = [];
  ...
  if (lesson) {
    ...
    for (let y = 0; y < H; y++)
      for (let x = 0; x < W; x++) {
        ...
        if (cell === 'b' || cell === 'k')
          state.enemies.push(makeEnemy(cell === 'b' ? 'bat' : 'knocker', x, y, state.rng));
  ...
  grid[key(3, 1)] = FLOOR;
  // Reserve a clear exit location; reveal it when the creatures are gone.
  ...
  // A launcher near the entrance introduces the secondary tool.
  ...
  const candidates = (minSize) => {
    ...
    for (let y = 1; y < H - 1; y++)
      for (let x = 1; x < W - 1; x++) {
        const k = key(x, y);
        if (grid[k] === FLOOR && x + y >= 7 && !used.has(k) && sizes[region[k]] >= minSize)
          out.push([x, y]);
      }
    return out;
  };
    ...
    for (let y = 1; y < H - 1; y++)
      for (let x = 1; x < W - 1; x++) {
        ...
        if (
          ...
          continue;
        const open = DIR_NAMES.filter(
          (d) => grid[key(x + DIRS[d][0], y + DIRS[d][1])] === FLOOR,
        ).length;
        out.push([x, y, open]);
      }
    out.sort((p, q) => q[2] - p[2]);
    return out
      .slice(0, Math.max(1, out.filter((c) => c[2] === out[0]?.[2]).length))
      .map((c) => [c[0], c[1]]);
  };
  ...
  state.shafts = [
    { x: 3, y: 1 },
    ...shuffled
      .filter(([x, y]) => grid[key(x, y)] === BRICK && !state.items.has(key(x, y)) && x + y >= 6)
      .slice(0, Math.min(3 + Math.floor(level / 4), 4) - 1)
      .map(([x, y]) => ({ x, y })),
  ];
...
export function startRound(state) {
  ...
  shuffled
    .slice(0, pool.length)
    .forEach(([x, y], i) => state.items.set(key(x, y), { type: pool[i], hidden: true }));
  state.shafts = shuffled
    .filter(([x, y]) => !state.items.has(key(x, y)))
    .slice(0, 3)
    .map(([x, y]) => ({ x, y }));
  for (const shaft of state.shafts) grid[key(shaft.x, shaft.y)] = FLOOR;
...
function moveEntity(state, ent, want, speed, dt) {
  ...
  while (dist > 1e-6 && guard-- > 0) {
    ...
    if (Math.abs(alignedOff) > 1e-6) {
      ...
      const tx = tileOf(cx) + dx,
        ty = tileOf(cy) + dy;
      if (blockedFor(state, ent, tx, ty)) return;
      const slide = Math.min(dist, Math.abs(alignedOff));
      if (horizontal) ent.y -= Math.sign(alignedOff) * slide;
      else ent.x -= Math.sign(alignedOff) * slide;
      if (Math.abs(alignedOff) <= dist + 1e-9) {
        if (horizontal) ent.y = cy;
        else ent.x = cx;
      }
      dist -= slide;
      ent.moving = true;
    ...
    const nextTx = tileOf(ent.x) + dx,
      nextTy = tileOf(ent.y) + dy;
    ...
    ent.facing = want;
    if (m >= toNext - 1e-9) {
      if (horizontal) ent.x = centre(ent.x);
      else ent.y = centre(ent.y);
      ent.atNode = true;
...
function playerSpeed(p) {
  return RULES.baseSpeed + p.speedItems * RULES.speedStep;
}
  ...
  for (const d of DIR_NAMES) {
    ...
    for (let i = 1; i <= range; i++) {
      ...
      cells.push([
        tx,
        ty,
        i === range || tileAt(state, tx + dx, ty + dy) === WALL ? 'end' : 'ray',
        d,
      ]);
      if (t === BRICK) break;
    }
...
export function blastCells(state, x, y, range) {
  ...
  for (const vent of vents)
    for (const cell of localBlastCells(state, vent.x, vent.y, 1)) {
...
export function usePower(state, p) {
  if (
    ...
    !p.power ||
    state.time >= p.powerUntil ||
    state.time < p.powerReady
  )
    return false;
...
function tickProjectiles(state, dt) {
  for (const shot of state.projectiles) {
    ...
    const samples = Math.max(1, Math.ceil(distance / 0.08));
    for (let i = 0; i < samples && shot.life > 0; i++) {
      shot.x += (shot.dx * distance) / samples;
      shot.y += (shot.dy * distance) / samples;
      const x = tileOf(shot.x),
        y = tileOf(shot.y);
      if (tileAt(state, x, y) !== FLOOR) {
        shot.life = 0;
        break;
      }
      ...
      const target = [...state.enemies, ...state.players.filter((p) => p.id !== shot.owner)].find(
        (e) =>
          e.alive &&
          !state.projectileHits.has(e) &&
          Math.abs(e.x - shot.x) < 0.42 &&
          Math.abs(e.y - shot.y) < 0.42,
      );
      if (target) {
        state.projectileHits.add(target);
        shot.life = 0;
        break;
      }
    }
...
function cpuUsePower(state, p) {
  ...
  if (p.power === ITEM.IGNITOR) {
    ...
    if (
      projected[ownCell] === Infinity &&
      cells.some(
        ([x, y]) => tileAt(state, x, y) === BRICK || (x === tileOf(foe.x) && y === tileOf(foe.y)),
      )
    )
      usePower(state, p);
  } else if (p.power === ITEM.FIREBALL) {
    ...
    if (!seesFoe) return;
    if (
      (dx && Math.abs(foe.y - p.y) < 0.35 && (foe.x - p.x) * dx > 0) ||
      (dy && Math.abs(foe.x - p.x) < 0.35 && (foe.y - p.y) * dy > 0)
    )
      usePower(state, p);
  }
}
...
export function dangerMap(state, extraBomb) {
  ...
  const bombs = state.bombs
    .filter((b) => !b.exploded)
    .map((b) => ({ x: b.x, y: b.y, range: b.range, t: b.at + b.fuse - state.time }));
  ...
  for (const shot of state.projectiles) {
    ...
    for (let distance = 0; distance <= length; distance += 0.1) {
      const x = tileOf(shot.x + shot.dx * distance),
        y = tileOf(shot.y + shot.dy * distance),
        k = key(x, y);
      if (tileAt(state, x, y) !== FLOOR) break;
      ...
      if (bomb) {
        bomb.t = Math.min(bomb.t, arrival);
        break;
      }
    }
  }
...
function cpuThink(state, p) {
  ...
  const mine = state.bombs.filter((b) => b.owner === p.id && !b.exploded).length;
  const canBomb =
    !state.projectiles.some((s) => s.owner === p.id) &&
    mine < p.maxBombs &&
    !bombAt(state, px, py) &&
    p.bombCooldown <= 0;
  if (canBomb && (hitsFoe || (hitsBrick && state.rng() < 0.85))) {
    const after = dangerMap(state, { x: px, y: py, range: p.range });
    const okAfter = (x, y, steps) =>
      after[key(x, y)] > steps * stepTime + 0.35 && !(x === px && y === py);
    const { dist } = bfs(state, px, py, p, okAfter);
    let escape = false;
    for (let k = 0; k < W * H && !escape; k++)
      if (dist[k] > 0 && after[k] === Infinity) escape = true;
    if (escape) {
      plan.bomb = true;
      return plan;
    }
  ...
  if (target === -1) {
    for (let k = 0; k < W * H; k++) {
      ...
      const x = k % W,
        y = (k - x) / W;
      const nearBrick = DIR_NAMES.some(
        (d) => tileAt(state, x + DIRS[d][0], y + DIRS[d][1]) === BRICK,
      );
      if (nearBrick && dist[k] < bestD) {
        bestD = dist[k];
        target = k;
      }
    }
  ...
  if (target !== -1) plan.dir = firstStep(prev, here, target);
    ...
    const opts = DIR_NAMES.filter((d) => {
      const nx = px + DIRS[d][0],
        ny = py + DIRS[d][1];
      return (
        tileAt(state, nx, ny) === FLOOR &&
        !bombAt(state, nx, ny) &&
        danger[key(nx, ny)] === Infinity
      );
    });
...
function enemyThink(state, e, danger = null) {
  ...
  if (danger) {
    const arrival = 1 / Math.max(e.speed, 0.1);
    const safeStep = (x, y, steps) => danger[key(x, y)] > steps * arrival + 0.18;
    ...
    if (danger[here] < Infinity) {
      ...
      let safest = -1;
      for (let k = 0; k < W * H; k++)
        if (dist[k] > 0 && danger[k] === Infinity && (safest < 0 || dist[k] < dist[safest]))
          safest = k;
    ...
    else if (danger[here] === Infinity)
      return null; // Wait instead of entering a blast.
    else
      return open.reduce((best, d) =>
        danger[key(ex + DIRS[d][0], ey + DIRS[d][1])] >
        danger[key(ex + DIRS[best][0], ey + DIRS[best][1])]
          ? d
          : best,
      );
  }
  ...
  if (state.level >= 8 && target) {
    let tx = tileOf(target.x),
      ty = tileOf(target.y);
    ...
    if (state.level >= 10 && target.moving && (ex + ey) % 2 === 0) {
      ...
      if (tileAt(state, tx + dx, ty + dy) === FLOOR && !bombAt(state, tx + dx, ty + dy)) {
        tx += dx;
        ty += dy;
      }
    }
    const route = bfs(
      ...
      (x, y, steps) =>
        !fireAt(state, x, y) &&
        (!danger || danger[key(x, y)] > steps / Math.max(e.speed, 0.1) + 0.18),
    );
    let destination = key(tx, ty);
    if (route.dist[destination] < 0) destination = key(tileOf(target.x), tileOf(target.y));
    if (route.dist[destination] > 0) {
      const d = firstStep(route.prev, here, destination);
      if (d && open.includes(d)) return d;
    }
  }
  if (
    (spec.style === 'hunter' || state.level >= 3) &&
    target &&
    state.rng() < (state.level >= 3 ? 0.9 : 0.8)
  ) {
    // Line of sight along a row or column → give chase.
    const tx = tileOf(target.x),
      ty = tileOf(target.y);
    for (const d of open) {
      const [dx, dy] = DIRS[d];
      let x = ex + dx,
...
export function step(state, dt, inputs = []) {
  ...
  if (state.status === 'intro' || state.status === 'cleared' || state.status === 'roundEnd') {
    if (state.time >= state.statusUntil) {
      if (state.status === 'intro') {
        ...
        if (champ) {
          state.status = 'over';
          state.events.push({ type: 'matchOver', winner: champ.id });
  ...
  for (const p of state.players) {
    ...
    if (bomb) {
      if (placeBomb(state, p)) p.bombCooldown = 0.25;
    }
    ...
    if (p.cpu) cpuUsePower(state, p);
    else if (power) usePower(state, p);
    // Pick up items
    ...
    if (
      state.door &&
      state.door.open &&
      tileOf(p.x) === state.door.x &&
      tileOf(p.y) === state.door.y
    ) {
      const bonus = RULES.levelBonus;
      addScore(state, bonus);
      state.status = 'cleared';
      state.statusUntil = state.time + 2.4;
  ...
  const cautious =
    state.level >= 5 && (state.bombs.length || state.projectiles.length || state.fires.size);
  // Enemies
  for (const e of state.enemies) {
    ...
    if (e.atNode || !e.moving || (cautious && state.time >= (e.rethinkAt || 0))) {
      ...
      e.rethinkAt = state.time + Math.max(0.12, 0.35 - (state.level - 5) * 0.035);
    }
...
function resolveHits(state) {
  ...
  for (const p of state.players) {
    ...
    if (!hit) {
      for (const e of state.enemies) {
        if (!e.alive) continue;
        if (Math.abs(e.x - p.x) < 0.6 && Math.abs(e.y - p.y) < 0.6) {
          hit = e.type;
          break;
        }
      }
    ...
    if (!p.movedSinceSpawn && state.time < p.graceUntil && hit !== 'crushed' && hit !== 'fire')
      continue;
  ...
  if (state.mode === 'battle' && state.status === 'playing') {
    ...
    if (alive.length <= 1) {
      ...
      state.statusUntil = state.time + 2.6;
      state.events.push({
        type: 'roundEnd',
...
function applyItem(state, p, type) {
  ...
  else if (type === ITEM.FIREBALL || type === ITEM.IGNITOR) {
    p.power = type;
    p.powerUntil = state.time + RULES.powerDuration;
    p.powerReady = state.time;
  }
...
function tickSuddenDeath(state, dt) {
  ...
  while (sd.clock >= RULES.suddenDeathStep && sd.next < SPIRAL.length) {
    ...
    for (const e of state.enemies)
      if (e.alive && tileOf(e.x) === x && tileOf(e.y) === y) {
        e.alive = false;
        e.deadAt = state.time;
      }
