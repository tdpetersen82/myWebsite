...
for(const dt of [8,16,33]) {
 const b=air();step(b,1000,{left:true},dt);const angle=b.angle,rate=b.flipVelocity;
 assert(rate < -400,'tuck builds pitch momentum');
 step(b,40,{},dt);assert.equal(b.angle,angle,'release stops arrow rotation for landing');
...
const short=air();step(short,120,{left:true});step(short,700,{});
assert(!short.tricks.includes('backflip'),'tap cannot auto-complete a flip');assert(Math.abs(short.angle)>15,'no automatic snap back');
for(const mode of ['whip','backflip','combo','tailwhip']) {
 ...
 for(let i=0;i<1200&&!landed;i++) {
  ...
  if(b.airborne) {
   if((mode==='backflip'||mode==='combo')&&!b.tricks.includes('backflip')) {input.left=true;input.right=false;}
