/* eslint-disable */
// Three Card Poker — lightweight Web Audio sound effects.
let _ctx = null;
function ctx() {
...
const TCP_SFX = {
  ...
  bonus:() => { [523, 659, 784, 1047, 1319].forEach((f, i) => setTimeout(() => tone({ freq: f, type: 'sine', dur: .22, gain: .18 }), i * 95)); },
  lose: () => { tone({ freq: 380, type: 'sawtooth', dur: .3, gain: .12, slideTo: 180 }); },
  push: () => { tone({ freq: 440, type: 'sine', dur: .12, gain: .1 }); },
  fold: () => { tone({ freq: 220, type: 'sawtooth', dur: .25, gain: .1, slideTo: 110 }); }
};

window.TCP_SFX = TCP_SFX;
