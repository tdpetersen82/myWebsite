class TrackRenderer {
    ...
    _renderGround() {
        ...
        // Subtle noise texture - scattered dots
        ...
        // Grass tufts in non-track areas
        for (let i = 0; i < 200; i++) {
    ...
    // --- TRACK SHADOW (depth under track surface) ---
    _renderTrackShadow() {
        const g = this.scene.add.graphics().setDepth(0.5);
        const n = this.centerPoints.length;
        const expand = 6;
        // Draw a slightly larger dark shape behind the track
    ...
    // --- TRACK SURFACE: multi-layer with edge shading ---
    _renderTrackSurface() {
        ...
        // Darker edges (inner shadow) - draw narrower lighter strip on top
        ...
        // Even lighter racing line center strip
        ...
        const grainGfx = this.scene.add.graphics().setDepth(1.3);
        const seed = this.trackIndex * 2000;
        for (let i = 0; i < 400; i++) {
            const idx = Math.floor(this._hash(seed + i) * n);
            const cp = this.centerPoints[idx];
            if (!cp) continue;
            const offX = (this._hash(seed + i + 1000) - 0.5) * this.trackData.trackWidth * 0.8;
            const offY = (this._hash(seed + i + 2000) - 0.5) * this.trackData.trackWidth * 0.8;
            grainGfx.fillStyle(this._hash(seed + i + 3000) > 0.5 ? 0x000000 : 0xFFFFFF, 0.04);
            grainGfx.fillCircle(cp.x + offX, cp.y + offY, 1 + this._hash(seed + i + 4000) * 2);
        }
    }

    _fillTrackPolygon(gfx, scale) {
        const n = this.centerPoints.length;
        gfx.beginPath();
        for (let i = 0; i < n; i++) {
    ...
    // --- RACING LINE (faded tire marks) ---
    _renderRacingLine() {
        const g = this.scene.add.graphics().setDepth(1.4);
        const n = this.centerPoints.length;
        // Two parallel dark lines along center
    ...
    // --- 3D CURBING (F1-style beveled kerbs) ---
    _renderCurbing() {
        ...
        for (let i = 0; i < n; i++) {
            const j = (i + 1) % n;
            const dx = this.leftEdge[j].x - this.leftEdge[i].x;
            const dy = this.leftEdge[j].y - this.leftEdge[i].y;
            ...
            // Highlight bevel on inner edge
            g.lineStyle(1, this._lighten(color, 0.4), 0.5);
            g.beginPath(); g.moveTo(x1, y1); g.lineTo(x4, y4); g.strokePath();
        }
        ...
        for (let i = 0; i < n; i++) {
            const j = (i + 1) % n;
            const dx = this.rightEdge[j].x - this.rightEdge[i].x;
            const dy = this.rightEdge[j].y - this.rightEdge[i].y;
    ...
    // --- TERRAIN OVERLAYS (organic shapes with texture) ---
    _renderTerrainOverlays() {
        ...
        for (const zone of this.terrainZones) {
            ...
            if (zone.type === 'WATER') {
                ...
                g.fillEllipse(cx, cy, r * 2.1, r * 1.7);
                g.fillStyle(terrain.color, 0.6);
                g.fillEllipse(cx - 2, cy - 2, r * 1.9, r * 1.5);
                // Highlight sheen
                g.fillStyle(0xAED6F1, 0.2);
                g.fillEllipse(cx - r * 0.2, cy - r * 0.25, r * 1.0, r * 0.7);
                // Ripple rings
                g.lineStyle(1, 0xFFFFFF, 0.15);
                for (let rv = 8; rv < r; rv += 10) {
                    g.strokeEllipse(cx + Math.sin(rv) * 3, cy, rv * 1.8, rv * 1.4);
                }
            } else if (zone.type === 'MUD') {
                // Organic mud shape
                g.fillStyle(0x3E2723, 0.7);
                g.fillEllipse(cx, cy, r * 2.2, r * 1.8);
                g.fillStyle(terrain.color, 0.6);
                g.fillEllipse(cx + 3, cy + 2, r * 1.9, r * 1.5);
                // Mud bubbles
                for (let i = 0; i < 12; i++) {
                    ...
                    g.fillStyle(0x2C1A0E, 0.5);
                    g.fillCircle(bx, by, bs);
                    g.fillStyle(0x4E342E, 0.3);
                    g.fillCircle(bx - 1, by - 1, bs * 0.6);
                }
            } else if (zone.type === 'ICE') {
                g.fillStyle(0xD6EAF8, 0.6);
                g.fillEllipse(cx, cy, r * 2.1, r * 1.7);
                g.fillStyle(terrain.color, 0.5);
                g.fillEllipse(cx, cy, r * 1.8, r * 1.5);
                // Crystal highlights
                ...
                g.fillEllipse(cx, cy, r * 2, r * 1.6);
                g.fillStyle(terrain.color, 0.9);
                g.fillEllipse(cx, cy, r * 1.7, r * 1.3);
                // Plank lines
                const dir = zone.direction;
                g.lineStyle(2, 0x6B4F12, 0.5);
                for (let i = -2; i <= 2; i++) {
                    const perpX = Math.cos(dir + Math.PI/2) * i * 6;
                    const perpY = Math.sin(dir + Math.PI/2) * i * 6;
                    g.beginPath();
                ...
                for (let i = -1; i <= 1; i++) {
                    this._drawArrow(g, cx + Math.cos(dir) * i * 10, cy + Math.sin(dir) * i * 10, dir, 8);
                }
            }
        }
    }

    _drawArrow(gfx, x, y, angle, size) {
        const tipX = x + Math.cos(angle) * size, tipY = y + Math.sin(angle) * size;
        const lx = x + Math.cos(angle + 2.5) * size * 0.7, ly = y + Math.sin(angle + 2.5) * size * 0.7;
        const rx = x + Math.cos(angle - 2.5) * size * 0.7, ry = y + Math.sin(angle - 2.5) * size * 0.7;
    ...
    _renderCheckpoints() {
        ...
        for (let i = 1; i < cps.length; i++) {
            ...
            for (let s = 0; s < steps; s += 2) {
                ...
                const x0 = cp.x + Math.cos(perpAngle) * len * (1 - 2 * t0);
                const y0 = cp.y + Math.sin(perpAngle) * len * (1 - 2 * t0);
                const x1 = cp.x + Math.cos(perpAngle) * len * (1 - 2 * t1);
                const y1 = cp.y + Math.sin(perpAngle) * len * (1 - 2 * t1);
    ...
    // --- FINISH LINE (premium checkered with glow) ---
    _renderFinishLine() {
        ...
        const startX = cp.x + Math.cos(perpAngle) * halfWidth;
        const startY = cp.y + Math.sin(perpAngle) * halfWidth;
        const endX = cp.x - Math.cos(perpAngle) * halfWidth;
        const endY = cp.y - Math.sin(perpAngle) * halfWidth;
        ...
        for (let r = 3; r > 0; r--) {
            g.fillRoundedRect(
                Math.min(startX, endX) - r * 4,
                Math.min(startY, endY) - checkerSize * 1.5 - r * 4,
                totalLen + r * 8,
                checkerSize * 3 + r * 8,
                4
            );
        ...
        // Checkered squares with 3D bevel
        for (let i = 0; i < steps; i++) {
            for (let j = 0; j < 3; j++) {
                ...
                g.fillRect(bx + 1, by + 1, checkerSize - 1, checkerSize - 1);
                // Face
                g.fillStyle(isWhite ? 0xF0F0F0 : 0x1A1A1A, 1);
                g.fillRect(bx, by, checkerSize - 1, checkerSize - 1);
                // Highlight
                if (isWhite) {
                    g.fillStyle(0xFFFFFF, 0.3);
                    g.fillRect(bx, by, checkerSize - 1, 2);
                }
            }
        }
    }

    // --- GROUND DETAILS (pebbles, grass near track edges) ---
    _renderGroundDetails() {
        const g = this.scene.add.graphics().setDepth(0.8);
        const n = this.centerPoints.length;
        const seed = this.trackIndex * 5000;

        // Pebbles near track edges
        for (let i = 0; i < 150; i++) {
            const idx = Math.floor(this._hash(seed + i) * n);
            const cp = this.centerPoints[idx];
            if (!cp) continue;
            const prev = this.centerPoints[(idx - 1 + n) % n];
            ...
            const px = cp.x + ux * dist * side;
            const py = cp.y + uy * dist * side;

            const size = 1.5 + this._hash(seed + i + 300) * 2.5;
            g.fillStyle(this._darken(this.theme.ground, 0.2), 0.4);
            g.fillCircle(px + 0.5, py + 0.5, size);
            g.fillStyle(this._lighten(this.theme.ground, 0.15), 0.5);
            g.fillCircle(px, py, size * 0.8);
        }
    }

    // --- DECORATIONS (premium 3D-shaded objects) ---
    _renderDecorations() {
    ...
    _drawRock(x, y) {
        ...
        g.fillStyle(0x9A9A9A, 0.4); g.fillEllipse(x - 5, y - 5, 8, 5);
        g.lineStyle(1, 0x4A4A4A, 0.3);
        g.beginPath(); g.moveTo(x - 6, y + 2); g.lineTo(x + 3, y - 5); g.strokePath();
    }
    ...
    _drawTireStack(x, y) {
        ...
        const drawTire = (tx, ty) => {
            ...
            g.lineStyle(1, 0x3A3A3A, 0.8); g.strokeCircle(tx, ty, 7);
            ...
            for (let a = 0; a < 6; a++) {
                ...
                g.beginPath();
                g.moveTo(tx + Math.cos(angle) * 5, ty + Math.sin(angle) * 5);
                g.lineTo(tx + Math.cos(angle) * 9, ty + Math.sin(angle) * 9);
                g.strokePath();
            }
        };
        drawTire(x, y + 2);
    ...
    _drawSpectatorStand(x, y, width) {
        ...
        // Spectators (with bodies)
        const colors = [0xFF3333, 0x3388FF, 0x33CC33, 0xFFCC00, 0xFF66AA, 0xFFFFFF, 0xFF8833, 0x9933FF];
        ...
        for (let i = 0; i < count; i++) {
            ...
            g.fillRoundedRect(sx - 3, y - 12, 6, 10, 2);
            // Head
            g.fillStyle(0xFFDBB5, 1); g.fillCircle(sx, y - 16, 4);
            // Hair
            g.fillStyle(this._darken(color, 0.3), 1); g.fillCircle(sx, y - 18, 3);
        }
        // Roof
        g.fillStyle(0x37474F, 0.8);
        g.fillRoundedRect(x - width / 2 - 5, y - 24, width + 10, 4, 2);
        g.fillStyle(0x455A64, 0.6);
        g.fillRect(x - width / 2 - 5, y - 24, width + 10, 2);
        // Roof supports
        g.fillStyle(0x546E7A, 1);
        g.fillRect(x - width / 2, y - 24, 2, 10);
        g.fillRect(x + width / 2 - 2, y - 24, 2, 10);
    }
    ...
    _drawPine(x, y) {
        ...
        g.fillStyle(0x5D4037, 1); g.fillRect(x - 1, y - 4, 2, 12);
        // Canopy layers (bottom up, getting smaller)
        const layers = [
            { w: 26, h: 14, yOff: -8 },
            { w: 22, h: 13, yOff: -16 },
            { w: 16, h: 12, yOff: -24 },
    ...
    _drawSnowman(x, y) {
        ...
        g.fillStyle(0xF5F5F5, 1); g.fillCircle(x - 1, y - 14, 5.5);
        // Head
        g.fillStyle(0xE0E0E0, 1); g.fillCircle(x, y - 22, 5);
        g.fillStyle(0xF5F5F5, 1); g.fillCircle(x - 0.5, y - 22.5, 4);
        // Carrot nose
        ...
        g.fillRect(x - 6, y - 28, 12, 2);
        g.fillRect(x - 4, y - 35, 8, 7);
        // Scarf
        g.fillStyle(0xE53935, 1);
        g.fillRoundedRect(x - 5, y - 17, 10, 3, 1);
        g.fillRoundedRect(x + 3, y - 17, 3, 7, 1);
        // Buttons
        g.fillStyle(0x212121, 1);
        g.fillCircle(x, y - 4, 1.2); g.fillCircle(x, y + 2, 1.2);
    }
    ...
    _drawPalm(x, y) {
        ...
        g.fillRoundedRect(x - 4, y - 32, 8, 34, 3);
        // Trunk rings
        g.lineStyle(1, 0x5D4037, 0.4);
        for (let ty = y - 28; ty < y; ty += 5) {
            g.beginPath(); g.moveTo(x - 3, ty); g.lineTo(x + 3, ty); g.strokePath();
        }
        g.fillStyle(0x795548, 0.5); g.fillRect(x - 2, y - 30, 4, 30);
        // Fronds
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const mid1x = x + Math.cos(angle) * len * 0.4;
            const mid1y = y - 34 + Math.sin(angle) * 3;
            g.fillStyle(frondColors[i % frondColors.length], 1);
            g.fillTriangle(x, y - 32, mid1x - 2, mid1y, fx, fy);
            g.fillTriangle(x, y - 32, mid1x + 2, mid1y, fx + 2, fy + 2);
            // Frond detail line
            g.lineStyle(1, 0x1B5E20, 0.3);
            g.beginPath(); g.moveTo(x, y - 32); g.lineTo(fx, fy); g.strokePath();
        }
        // Coconuts
        g.fillStyle(0x5D4037, 1);
        g.fillCircle(x - 3, y - 30, 3); g.fillCircle(x + 2, y - 29, 2.5);
    }
    ...
    _drawBush(x, y) {
        ...
        // Highlight
        g.fillStyle(0x66BB6A, 0.4); g.fillEllipse(x - 2, y - 4, 6, 4);
    }
