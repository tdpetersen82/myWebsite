...
// video poker, and three card poker. Stored in localStorage under
// "casinoBankroll" (number).
//
// One-shot migration: if no casinoBankroll is present but a legacy per-game
// bankroll exists in rouletteStats / videoPokerStats / threeCardPokerStats,
// seed the shared key from the max of those values so a returning player
...
(function () {
  ...
  function migrateIfNeeded() {
    ...
    // Take the highest legacy bankroll across games that used to persist their
    // own. If none have one, fall back to STARTING.
    let legacy = 0;
    try {
    ...
    try {
      ...
      if (tcp && typeof tcp.bankroll === 'number' && tcp.bankroll > 0) {
        legacy = Math.max(legacy, tcp.bankroll);
      }
    } catch (e) {}
